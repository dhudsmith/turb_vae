...
class VAETrainer(pl.LightningModule):
    def __init__(self, encoder: Encoder2d, decoder: Decoder2d, kl_weight: float = 1.0, learning_rate: float = 1e-3):
        super().__init__()
    ...
    def get_losses(self, batch):
        n, _ = batch
        n_hat, mu, log_var = self.vae(n)

        recon_loss = torch.nn.functional.mse_loss(n_hat, n)
        kl_loss = self.vae.KL_divergence(mu, log_var)
